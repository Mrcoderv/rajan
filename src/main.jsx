...
function InstagramEmbed({ postUrl }) {
  if (!postUrl || postUrl.startsWith('REPLACE_WITH_')) {
    return <a className="instagram-fallback" href="https://www.instagram.com/rjnvibe/" target="_blank" rel="noreferrer">View on Instagram <span>↗</span></a>
  }

  return <div className="feed-frame instagram-frame"><iframe src={`https://www.instagram.com/p/${postUrl}/embed/`} loading="lazy" title={`Instagram post ${postUrl}`} /></div>
}
...
function App() {
  ...
  return <div className="site-shell">
    ...
    <main id="top">
      ...
      <section className="content-section" id="work"><div className="section-heading"><div><p className="section-kicker">02 / Certifications</p><h2>Proof of <em>practice.</em></h2></div><p className="section-aside">A growing collection of credentials and milestones from my learning journey.</p></div><div className="certificate-grid">{content.certificates.map((certificate) => <article className="certificate" key={certificate.title}>{certificate.image && <a className="certificate-image-link" href={`/${certificate.image}`} target="_blank" rel="noreferrer"><img className="certificate-image" loading="lazy" src={`/${certificate.image}`} alt={certificate.title} /></a>}<div><div className="certificate-top"><span>{certificate.issuer}</span><span>{certificate.date}</span></div>{certificate.issuerDetail && <p className="issuer-detail">{certificate.issuerDetail}</p>}<h3>{certificate.title}</h3>{certificate.credentialId && <p>Credential ID: {certificate.credentialId}</p>}</div>{certificate.url && <a className="credential" href={certificate.url} target="_blank" rel="noreferrer">View credential ↗</a>}</article>)}</div></section>
